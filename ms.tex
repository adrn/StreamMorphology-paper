...
\end{equation}
\chchchanges{where $q_k$ and $p_k$ are conjugate coordinates in the sense that
\begin{align}
...
Arnold diffusion is not expected to be significant for most orbits over timescales relevant to galaxies (10s of orbits), however chaotic motion across resonances can occur over short times. \chchchanges{If a stochastic trajectory is surrounded by regular orbits, it may appear similar to a nearby regular orbit (the `lead` orbit) over long periods of time before escaping to another such semi-bounded region where it will change in appearance to `trace' a new lead orbit (a process by which it may eventually explore the whole Arnold web). However, two initially nearby trajectories may switch to  different lead orbits at different times, a process which is clearly important for the evolution of tidal debris. Additionally, if the volume of the surrounded region (e.g., in frequency space) is comparable to the characteristic spread of frequencies in the tidal debris, this small-scale evolution will be important for tidal debris.}
