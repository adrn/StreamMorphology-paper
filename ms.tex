...
We now repeat the above analysis of the short-time evolution of frequencies, but now for ensembles of orbits around each of the three parent orbits. [TODO: a new metric, measuring the frequency variance, Figure~\ref{fig:freqvar_map} ...]

\section{Discussion}\label{sec:discussion}

Since all known tidal streams have been discovered as kinematic over-densities of stars, this implies (1) [something about accretion history], (2) the long, thin tidal streams we see may represent a sampling from ``special'' orbits in the Galactic potential where the mixing time is long, and (3) previous assumptions that connect the length and size of streams to their evolution time may be invalid.
...
\begin{figure*}[p]
\begin{center}
\includegraphics[width=\textwidth]{figures/pal5-lyap.png}
\caption{ FTMLE (Equation~\ref{eq:ftmle}) computed over XXX orbital periods for the stream-fanning Palomar 5 orbit of \cite{pearson15}. The FTMLE saturates to the Lyapunov exponent in the shaded region and is estimated to be $\lambda_{\rm max} \approx XXX \, {\rm Gyr}^{-1}$, corresponding to a Lyapunov time of $t_\lambda \approx XX \, {Gyr}$. [TODO: add labels, remake with better font, remove blue line, add axis labels, add shaded region, check on all numbers for paragraph] } \label{fig:pal5}
\end{center}
\end{figure*}
