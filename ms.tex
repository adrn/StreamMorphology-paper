...
For example, on a mildly chaotic orbit with a chaotic timescale predicted to be $>$1000 orbital periods ($>$1000 Gyr), the resulting stellar stream is, after just 16 orbits, substantially more diffuse than any formed on a nearby but regular orbit.
We find that the enhanced diffusion of the stream stars can be estimated by looking at the variance in orbital frequencies along the parent (progenitor) orbit.
...
\begin{equation}
	\mathcal{R}_k = \frac{\Omega_{k}^{(2)} - \Omega_{k}^{(1)}}{\Omega_{k}^{(1)}} \label{eq:fdrate}
\end{equation}
where the upper index refers to the two consecutive sections of orbit \citep{laskar93, valluri98, valluri12}. By inverting this rate, we can compute the timescale over which we expect order-unity changes to the fundamental frequencies: the \emph{frequency diffusion time} is defined as
\begin{equation}
...
\begin{center}
	\begin{tabular}{c | c c c c c c | c c }
		{\bf ID} & $\bs{x}$ & $\bs{y}$ & $\bs{z}$ & $\bs{v_x}$ & $\bs{v_y}$ & $\bs{v_z}$ & $\bs{t_\lambda/\periods}$ & $\bs{t_\Omega/\periods}$ \\\toprule
A & 17 & 0 & 26.06 & 0 & 126.25 & 0 & $>$10^8 & XX\\
\midrule
B & 17 & 0 & 23.03 & 0 & 148.61 & 0 & XX & XX\\
\midrule
C & 17 & 0 & 25.35 & 0 & 131.68 & 0 & XX & XX\\
\midrule
D & 17 & 0 & 28.69 & 0 & 104.54 & 0 & XX & XX\\
		\bottomrule
		\end{tabular}
	\caption{Orbits from the $xz$ grid with a range of chaotic timescales---A is a regular, near-resonant orbit, B a regular, non-resonant orbit, C a weakly chaotic orbit, and S a strongly chaotic orbit. Initial conditions are given in physical units: Positions ($x$, $y$, $z$) are given in kpc, velocities ($v_x$, $v_y$, $v_z$) in km~s$^{-1}$. Chaotic timescales are expressed ($t_\lambda$, $t_\Omega$) in number of orbital periods. Recall that the frequency diffusion time, $t_\Omega$, is the time over which we expect order-unity changes in the fundamental frequencies, hence why the timescales appear quite long. \label{tbl:orbit-info}}
\end{center}
\end{table*}
...
\centering
    ...
    \subfloat[Next sub-figure\label{subfig-2:dummy}]{
      \includegraphics[width=0.45\textwidth]{figures/apocenter-map.png}
    }
\caption{ A grid of isoenergy orbits initialized on the $xz$ plane. Each pixel in these panels represents a single orbit, and the shading of each pixel corresponds to the median pericentric distance (left) or median apocentric distance (right) computed over 16 orbital periods. The central black band in the left panel and white band in the right panel are tube orbits with apocenter-to-pericenter ratios close to one. }
\label{fig:apoper}
\end{figure}
